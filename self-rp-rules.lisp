...
(defmethod <parse-token> ((p parser)) ;; predicate
  (cond
   ((parser-success-p (look-char? p #\'))(input-char p #\')(input p :CHARACTER)(input-char p #\')(return-from <parse-token> :ok));choice clause
   ( (look-upcase-symbol? p)
     ...
     (return-from <parse-token> :ok)
     );choice alt
   ( t 
     (return-from <parse-token> :fail)
     );choice alt
   );choice
